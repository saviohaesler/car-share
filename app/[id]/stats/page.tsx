    ...
    <main className="w-full h-full flex flex-col items-center px-4 pb-4 pt-[calc(1rem+env(safe-area-inset-top))] bg-gray-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 overflow-y-auto relative transition-colors duration-200">
      ...
      {isDetailModalOpen && selectedLog && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[100] p-4 pb-[calc(4rem+env(safe-area-inset-bottom)+1rem)] animate-in fade-in duration-200">
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] w-full max-w-sm border border-gray-100 dark:border-zinc-800 shadow-2xl flex flex-col max-h-full overflow-hidden">
            ...
            <div className="flex flex-col gap-4 overflow-y-auto flex-1 min-h-0 px-1 pb-2 text-left text-zinc-900 dark:text-zinc-150 custom-scrollbar">
              ...
              <div className="flex flex-col gap-2 mt-2">
                {selectedLog.source === 'auto' && (
                    <Link href={`/${resolvedParams.id}/map?routeId=${selectedLog.id}`} className="w-full bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 font-bold py-4 rounded-2xl uppercase text-xs active:scale-95 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition text-center border border-blue-100 dark:border-blue-900/30 flex items-center justify-center gap-2">
